...
本工作流集成了以下增强工具，**Stage 1 和 Stage 2 必须优先使用**：

| 工具 | 用途 | 调用方式 |
|------|------|----------|
| **Exa 语义搜索** | 深度语义搜索，结果比 web_search 更精准 | `mcporter call exa.search '{"query": "...", "numResults": 10}'` |
| **小红书 MCP** | 直接获取小红书一手热点、笔记、评论 | `mcporter call xiaohongshu.search_notes '{"keyword": "..."}'` |
| **agent-reach** | 将任意网页 URL 转为结构化文本 | `agent-reach read "{url}"` |
| **yt-dlp** | 提取 YouTube/Bilibili 视频字幕 | `yt-dlp --write-auto-sub --skip-download "{url}"` |

**优先级**：增强工具 > web_search/web_fetch。增强工具失败时自动降级为内置工具。
...
**情况 C：用户没有主题**
- Topic Scout 使用增强工具链扫描热点平台：
  - 优先用「小红书 MCP」获取小红书一手热点数据
  - 优先用「Exa 语义搜索」进行全网深度搜索
  - 用「web_search」补充搜索微博、知乎、抖音等平台
  - 用「agent-reach」读取具体帖子/文章全文
...
**Step 2: 执行素材收集（增强版 - 多工具协同）**

Researcher 按照提示词要求，综合使用增强工具链：

**核心工具（优先使用）：**
- 使用 `mcporter call exa.search` 进行深度语义搜索（替代部分 web_search）
- 使用 `mcporter call xiaohongshu.search_notes` 获取小红书一手数据
- 使用 `agent-reach read` 读取具体文章全文（替代部分 web_fetch）
- 使用 `yt-dlp` 提取 YouTube/Bilibili 视频字幕作为素材

**基础工具（补充使用）：**
- 使用 web_search 搜索微博、知乎、抖音等暂无 MCP 的平台
- 使用 web_fetch 作为 agent-reach 的备用方案
...
- 标注使用的工具名称
- 整理成结构化素材库
- 如果增强工具调用失败，自动降级为 web_search/web_fetch
