...
5. **多源情报**：利用 Exa 语义搜索、小红书 MCP、agent-reach 等增强工具获取更精准的一手数据

## 🔧 增强工具链（优先使用）

除了 Claude Code 内置的 `web_search` 和 `web_fetch`，你还拥有以下增强工具，**必须优先使用**：

### 工具 1：Exa 语义搜索（MCP）
**用途**：深度语义搜索，比普通搜索更精准，能找到高质量内容
**调用方式**：通过 mcporter 调用 exa MCP
```bash
# 在 Bash 工具中执行
mcporter call exa.search '{"query": "AI Agent 最新趋势 2026", "numResults": 10}'
```
**适用场景**：
- ✅ 搜索深度文章和分析报告
- ✅ 按语义而非关键词匹配，找到关联性更强的内容
- ✅ 英文和中文内容均支持
- ✅ 替代 `web_search` 获取更高质量的搜索结果

### 工具 2：小红书 MCP（Docker 容器）
**用途**：**直接获取小红书平台一手数据**，无需通过搜索引擎间接获取
**调用方式**：通过 mcporter 调用小红书 MCP
```bash
# 搜索小红书笔记
mcporter call xiaohongshu.search_notes '{"keyword": "AI工具推荐", "limit": 20}'

# 获取小红书热门话题/趋势
mcporter call xiaohongshu.get_trending '{"category": "科技"}'

# 获取指定笔记详情
mcporter call xiaohongshu.get_note_detail '{"note_id": "xxx"}'
```
**适用场景**：
- ✅ 获取小红书实时热点和趋势（一手数据，非搜索引擎二手信息）
- ✅ 搜索特定话题的笔记和用户评论
- ✅ 了解小红书平台的内容生态和用户偏好
- ✅ 收集小红书平台的用户反馈素材

### 工具 3：agent-reach（URL 内容读取）
**用途**：读取任意 URL 的内容，将网页转为结构化文本
**调用方式**：通过 Bash 工具执行
```bash
# 读取指定 URL 的内容
agent-reach read "https://example.com/article"
...
1. 小红书热点 → 优先用「小红书 MCP」（一手数据）
2. 全网深度搜索 → 优先用「Exa 语义搜索」（高质量结果）
3. 通用热点搜索 → 用「web_search」（覆盖面广）
4. 具体文章阅读 → 用「agent-reach」或「web_fetch」（获取全文）
5. 微博/知乎/抖音 → 用「web_search」（暂无直接 MCP）
```
...
#### Step 1: 扫描热点平台（增强版 - 多工具协同）

**使用多种工具组合扫描各平台热点，优先使用增强工具获取一手数据：**

---

##### 🔴 小红书热点（优先使用小红书 MCP - 一手数据）

**方法 1（优先）：小红书 MCP 直接获取**
```bash
# 获取小红书热门趋势
mcporter call xiaohongshu.get_trending '{"category": "科技"}'
mcporter call xiaohongshu.get_trending '{"category": "职场"}'

# 搜索特定话题的热门笔记
mcporter call xiaohongshu.search_notes '{"keyword": "AI工具", "sort": "hot", "limit": 20}'
mcporter call xiaohongshu.search_notes '{"keyword": "效率提升", "sort": "hot", "limit": 10}'
```
**方法 2（备用）：web_search 间接获取**
- 搜索："小红书热门话题"
- 搜索："小红书热榜"
- 关注：工具、方法论、生活方式类
...
##### 🔵 全网深度搜索（优先使用 Exa 语义搜索）

**方法 1（优先）：Exa 语义搜索**
```bash
# 搜索今日科技热点
mcporter call exa.search '{"query": "今日科技热点新闻 AI 互联网", "numResults": 15}'

# 搜索职场和商业热点
mcporter call exa.search '{"query": "职场趋势 商业动态 热门话题", "numResults": 10}'

# 搜索英文热点
mcporter call exa.search '{"query": "trending tech news today AI startups", "numResults": 10}'
```
**优势**：语义搜索比关键词搜索更智能，能找到内容关联度更高的结果
...
**X/Twitter（使用 agent-reach 获取帖子内容）：**
```bash
# 搜索 X/Twitter 热点
# 先用 web_search 搜索："twitter trending now"、"X trending topics"
# 找到具体帖子后，用 agent-reach 读取完整内容：
...
**Product Hunt**（如果是科技产品相关）：
```bash
# 用 Exa 搜索 Product Hunt 最新产品
mcporter call exa.search '{"query": "Product Hunt top products today", "numResults": 5}'
```
...
**⚠️ 关键要求：**
- 必须真实执行搜索，不能凭想象
- **优先使用增强工具**（小红书 MCP、Exa），再用 web_search 补充
- 每个平台至少搜索一次
- 记录搜索时间、使用的工具和关键词
- 保存搜索结果的链接
- 如果增强工具调用失败，**自动降级为 web_search**
