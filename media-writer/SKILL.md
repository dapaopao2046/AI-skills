...
除了 Claude Code 内置的 `web_search` 和 `web_fetch`，本工作流还集成了以下增强工具以提升信息收集的质量和效率。**Stage 1（选题）和 Stage 2（素材收集）必须优先使用这些工具**。

| 工具 | 用途 | 调用方式 | 适用 Stage |
|------|------|----------|-----------|
| **Exa 语义搜索** | 深度语义搜索，比 web_search 更精准 | `mcporter call exa.search '{"query": "...", "numResults": 10}'` | Stage 1, 2 |
| **小红书 MCP** | 直接获取小红书平台一手数据（热点、笔记、评论） | `mcporter call xiaohongshu.search_notes '{"keyword": "..."}'` | Stage 1, 2 |
| **agent-reach** | 读取任意 URL 内容，将网页转为结构化文本 | `agent-reach read "{url}"` | Stage 1, 2 |
| **yt-dlp** | 提取 YouTube/Bilibili 视频字幕，转为文本素材 | `yt-dlp --write-auto-sub --skip-download "{video_url}"` | Stage 2 |

### 工具优先级规则

```
小红书数据     → 优先「小红书 MCP」（一手数据） > web_search "xxx 小红书"
深度文章搜索   → 优先「Exa 语义搜索」（高质量） > web_search
具体文章全文   → 优先「agent-reach」 > web_fetch
视频内容提取   → 使用「yt-dlp」（独占能力，其他工具无法替代）
微博/知乎/抖音 → 使用「web_search」（暂无直接 MCP）
通用搜索       → 使用「web_search」（覆盖面最广）

⚠️ 如果增强工具调用失败，自动降级为 web_search / web_fetch
```

详见各 Agent prompt 中的「增强工具链」章节。
